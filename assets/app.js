(() => {
  ...
  $('#heroKpis').innerHTML = [
    [D.meta.analysedRespondents,'adult respondents analysed',0],
    [D.meta.validPins,'local mapped points',0],
  ...
  // ---------- method copy from computed adult data ----------
  ...
  $('#sampleCaution').textContent=`This was an open online survey, not a random sample. ${Math.round(within1)}% of adult respondents live within a kilometre and ${Math.round(frequentPct)}% visit at least several times a week. None of the ${A.local_map.n_emotion} emotion tags shown on the local map are negative. Treat every figure here as describing engaged adult users, not as a measure of public opinion in Kalmar.`;
  $('#offsiteExcluded').textContent=A.local_map.n_offsite_excluded;
  $('#scaleMethod').innerHTML=`Each scale is the unweighted mean of its items. All <strong>${A.meta.n_responses}</strong> adult respondents answered all 35 Likert items. Reliability is reported as Cronbach’s α and McDonald’s ω. The eleven-item multispecies block is reported as two scales because parallel analysis and factor rotation indicate two distinct dimensions; both are highly reliable.`;
